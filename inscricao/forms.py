...
class RegistroEmpresaForm(forms.ModelForm):
    regional = forms.ModelChoiceField(queryset=Regional.objects.all(),
                                      help_text='Consulte o regulamento para saber em '
                                                'qual delas a sua região se enquadra.',
                                      required=True)
    nome = forms.CharField(label='Nome da Empresa', max_length=30,
                           help_text='Use o nome de fantasia simplificado da empresa, sem "Ltda", "Propaganda",'
                                     '"Comunicação", "Criação", "Promoções", etc.')
    area = forms.ModelChoiceField(label='Área de Atuação', queryset=Area.objects.all(),
                                  help_text='Escolha a que mais se aproxime da sua principal área de atuação.')
    cep = forms.CharField(max_length=9, required=True, widget=forms.TextInput(attrs={'data-mask':"00000-000"}),
                          help_text='Digite no formato 00000-000')
    C1_Telefone = forms.CharField(max_length=9, required=True,
                                  widget=forms.TextInput(attrs={'data-mask': "00000-0000"}))
    C2_Telefone = forms.CharField(max_length=9, required=True,
                                  widget=forms.TextInput(attrs={'data-mask': "00000-0000"}))
    ...
    VP_Telefone = forms.CharField(label='Telefone', max_length=9, required=True,
                                  widget=forms.TextInput(attrs={'data-mask':"00000-0000"}))
    ...
    C1_Telefone = forms.CharField(label='Telefone', max_length=9, required=False,
                                  widget=forms.TextInput(attrs={'data-mask':"00000-0000"}))
    ...
    C2_Telefone = forms.CharField(label='Telefone', max_length=9, required=False,
                                  widget=forms.TextInput(attrs={'data-mask':"00000-0000"}))
